...
- **vitest 4.1.10 → 5.0.0.** A major, but it needed no code changes — all 193 tests passed as-is. Most of the breaking changes do not apply here (no deprecated entry points, no unawaited `resolves`/`rejects`, no `toHaveTextContent`, no `sequential`), and the prerequisites were already met by Node 26 and Vite 8. Two silent behaviour changes are worth remembering. `clearMocks` now defaults to `true`, so mock call history is reset between tests where v4 preserved it — the shared config deliberately leaves the new default in place rather than setting it back to `false`, since the suite passes under it and per-test isolation is the better behaviour (the `restoreMocks: true` it does set is a separate knob). And fake timers now mock `Temporal` as well as `Date` — so `Temporal.Now` returns mocked time under `vi.useFakeTimers()`. The latter removes a real inconsistency, since v4 mocked `Date` while `Temporal` kept reading the wall clock.
